...
class DMBGN(BaseModel):
    def __init__(self, dnn_feature_columns, history_feature_list, target_emb_dim_aft=0, sequence_size=0,
        ...
        self.dnn = DNN(inputs_dim=int(self.compute_input_dim(self.dnn_feature_columns) - target_emb_dim_aft + sequence_size + 3),
    ...
    def get_gnn_emb(self, session_id, gnet, gprefix = ['b_', 'a_'], gactions = ['atc', 'ord']) :
        ...
        if session_id in self.hash_sid_uvg_graphs_dic.keys():
            ...
            if len(graph) > 0 :
                ...
                return gnn_score, raw_promotion_id, promotion_emb.unsqueeze(0).unsqueeze(0), sess_emb.unsqueeze(0).unsqueeze(0), True
    ...
    def forward(self, X, step = 0):
            ...
            if ((step % 10) / 10 < (1-self.hist_gnn_dropout)) is True : 
                ...
                for sids in target_session_ids:
                    ...
                    for session_id in sids:
                        gnn_score, raw_promotion_id, promotion_emb, sess_emb, flag = self.get_gnn_emb(session_id, gnet = self.gnet_before, gprefix = ['b_'])
                        target_sess_emb_agg = sess_emb if target_sess_emb_agg is None else torch.cat([target_sess_emb_agg, sess_emb], dim=1)
                        target_promotion_emb_agg = promotion_emb if target_promotion_emb_agg is None else torch.cat([target_promotion_emb_agg, promotion_emb], dim=1)   

                    target_sess_emb_agg_all = target_sess_emb_agg if target_sess_emb_agg_all is None else torch.cat([target_sess_emb_agg_all, target_sess_emb_agg], dim=0)
                    target_promotion_emb_agg_all = target_promotion_emb_agg if target_promotion_emb_agg_all is None else torch.cat([target_promotion_emb_agg_all, target_promotion_emb_agg], dim=0)
                ...
                for sids in hist_session_ids:
                    ...
                    for session_id in sids:
                        gnn_score, raw_promotion_id, promotion_emb, sess_emb, flag = self.get_gnn_emb(session_id, gnet = self.gnet)
                        hist_sess_emb_agg = sess_emb if hist_sess_emb_agg is None else torch.cat([hist_sess_emb_agg, sess_emb], dim=1)
                        hist_promotion_emb_agg = promotion_emb if hist_promotion_emb_agg is None else torch.cat([hist_promotion_emb_agg, promotion_emb], dim=1)   

                    hist_sess_emb_agg_all = hist_sess_emb_agg if hist_sess_emb_agg_all is None else torch.cat([hist_sess_emb_agg_all, hist_sess_emb_agg], dim=0)
                    hist_promotion_emb_agg_all = hist_promotion_emb_agg if hist_promotion_emb_agg_all is None else torch.cat([hist_promotion_emb_agg_all, hist_promotion_emb_agg], dim=0)
        ...
        keys_masks = torch.arange(max_length, device=self.device, dtype=keys_length.dtype).repeat(batch_size, 1)  # [B, T]
